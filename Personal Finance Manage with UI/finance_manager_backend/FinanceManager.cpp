...
void FinanceManager::loadBudgetsFromCSV(const std::string& filename) {
    ...
    if (!file.is_open()) {
        std::cerr << "Error opening budgets file: " << filename << std::endl;
        return;
    }
    ...
    while (std::getline(file, line)) {
        ...
        std::string month;
        double amount;
        if (std::getline(ss, month, ',') && ss >> amount) {
            budgets.push_back(Budget(amount));
        } else {
            std::cerr << "Error reading line in budgets file: " << line << std::endl;
        }
    }
...
void FinanceManager::loadExpensesFromCSV(const std::string& filename) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error opening expenses file: " << filename << std::endl;
        return;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (std::getline(ss, month, ',') && std::getline(ss, name, ',') && ss >> amount) {
            ...
            std::cerr << "Error reading line in expenses file: " << line << std::endl;
        }
    }
...
void FinanceManager::trackMonthlyFinances(const std::string& outputFilename) {
    ...
    for (const auto& currentMonth : monthOrder) {
        ...
        if (budgetIndex >= budgets.size()) {
            std::cerr << "Error: No budget available for month: " << currentMonth << std::endl;
            continue;
        }
